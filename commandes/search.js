...
zokou({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and ᴄᴀsᴇʏʀʜᴏᴅᴇs ᴍᴅ will extract the text for you.");
    }
...
zokou({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and ᴄᴀsᴇʏʀʜᴏᴅᴇs ᴍᴅ will generate it.");
    }
    const prompt = args.join(" ");
...
zokou({
  ...
  try {
    ...
    message.sendMessage(user, {
      ...
      'caption': "*powered by ᴄᴀsᴇʏʀʜᴏᴅᴇs ᴍᴅ v²"
    }, {
      'quoted': messageInstance
    });
  } catch (error) {
