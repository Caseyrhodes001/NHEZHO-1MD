...
zokou({
  ...
  try {
    ...
    // Try Gifted API
    downloadData = await getDownloadData(`https://apis.giftedtech.web.id/api/download/dlmp4?apikey=gifted&url=${encodeURIComponent(videoUrl)}&apikey=gifted-md`);
    if (downloadData.success) {
      ...
      if (downloadData.success) {
        ...
        downloadData = await getDownloadData(`https://bandahealimaree-api-ytdl.hf.space/api/ytmp3?url=${encodeURIComponent(videoUrl)}`);
        if (downloadData.success) {
          downloadUrl = downloadData.result.download_url;
          videoDetails = downloadData.result;
        }
